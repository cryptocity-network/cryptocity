<template>
  <BlockWrapper :block-background-color="backgroundColor" :padding-top="96" :no-padding-bottom="false"
    ...
    <div class="relative mx-auto max-w-screen-2xl">
      <HeadlineSection :label="data.headline.label" :headline="data.headline.headline"
    ...
    <div class="relative !mx-0 !max-w-unset !px-0">
      <div class="no-scrollbar relative w-full" v-bind="containerProps" @scroll.passive="calculateStep">
        <ul v-bind="locationsToShow.length > 0 ? wrapperProps : {}" ref="scroller" role="list"
          ...
          <li ref="slides" data-location
            ...
            <div
              ...
              <div
                ...
                <img class="h-full rounded-4 object-cover" width="356" height="305"
                  src="/static/map-business-background.png" alt="">
              ...
              <div class="flex grow flex-col items-center  justify-center gap-16 p-24 py-32">
                ...
                <TheLink :text="data.joinLinkLabel" url="https://map.nimiq.com/?modal=crypto-map&nested=candidate"
                  variant="info" is-external />
              </div>
            </div>
          </li>
          <li v-for="{ data: location, index: i } in locationsToShow" :key="`${location.name}-${i}`" ref="slides"
            class="w-[clamp(320px,370px,calc(100vw-40px))] shrink-0 snap-center snap-always" data-location
      ...
      <button v-if="activeIndex < (locations?.length || 0) - visibleCards"
    ...
    <div v-if="locations?.length > 0" class="mx-auto flex flex-col justify-center overflow-hidden !p-0"
      :style="`max-width: ${(indicatorsOnSide * 2 * 16) + ((visibleCards * 16) + 8)}px;`">
      <div class="relative mt-8 flex gap-8 transition-[left] duration-300"
        :style="`left: -${cardsHiddenOnLeft * 16}px;`">
        <button v-for="(_location, i) in locations" :key="i"
        ...
        <div class="pointer-events-none absolute h-8 w-full rounded">
          <div class="h-full rounded bg-radial-blue-light transition-all duration-300"
