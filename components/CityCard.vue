<template>
  <nuxt-link :to="cityLink" :aria-label="`To ${city.name} page`"
    class="custom-ease group relative block size-full min-h-full overflow-hidden rounded-8" :class="{
      'transform-gpu transition-transform hover:-translate-y-6': city.state === 'Live',
      'pointer-events-none': city.state !== 'Live'
    }">
    <DatoImage :image="city.mainImage" :priority="true" class="size-full object-cover" :class="{
      'opacity-75': city.state === 'Live',
      'opacity-50 grayscale': city.state !== 'Live'
    }" />
    <div class="absolute left-0 top-0 size-full rounded-8 mix-blend-hard-light" :class="{
      'bg-blue opacity-20': city.state === 'Live',
      'bg-blue opacity-30': city.state !== 'Live'
    }" />
    <div v-if="city.state === 'Live'"
      class="absolute left-0 top-0 size-full rounded-8 bg-gradient-to-t from-blue to-transparent opacity-60 transition-opacity group-hover:opacity-0" />
    <DynamicLogo type="cities" logo-color="#1F2348" text-color="#1F2348" :city-name="city.name"
      class="absolute left-1/2 top-1/2  -translate-x-1/2 -translate-y-1/2"
      :class="compact ? 'w-2/3' : 'w-[215px] xl:w-[291px]'" />
    <DynamicLogo type="cities" logo-color="white" text-color="white" :city-name="city.name"
      class="absolute left-1/2 top-1/2  -translate-x-1/2 -translate-y-1/2"
      :class="compact ? 'w-2/3' : 'w-[215px] xl:w-[291px]'" />
    <div class="absolute bottom-32 left-1/2 -translate-x-1/2">
      <div v-if="city.state !== 'Live'"
        ...
        <svg viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" style="height: 18px">
      ...
      <div v-else-if="locationCount" class="text-lg text-center leading-[100%] text-white">
        <Location class="mx-auto mb-12 h-32" />
        <strong>{{ locationCountStr }}</strong> <span class="opacity-60">{{ $t('Locations') }}</span>
      </div>
    </div>
  </nuxt-link>
</template>
...
const { loadLocationsByCity } = useWebsiteStore()
const { locationsCount } = storeToRefs(useWebsiteStore())
...
const formatter = new Intl.NumberFormat(locale.value, { notation: 'compact', maximumFractionDigits: 2 })
const locationCount = computed(() => locationsCount.value[props.city.name] || 0)
const locationCountStr = computed(() => formatter.format(locationCount.value))
</script>
