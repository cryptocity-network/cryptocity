<template>
    <div class="child:aspect-video child:shadow child:rounded-6 mobile-wide"
      :class="{ 'bg-video': !hasPoster }">
      <div v-if="!showingYoutubeIframe" class="flex overflow-hidden relative items-end shadow child:rounded-6">
        <h2 
					v-if="showHeadline"
        ...
        <template v-if="!hasPoster">
          <div class="absolute w-full h-full opacity-10" style="
        ...
        <button class="grid absolute top-0 left-0 place-items-center w-full h-full drop-shadow-md z-1"
          @click="showYoutubeIframe">
          <!-- FIXME Now we use scale to make it responsive... -->
          <PlayButton filled class="!lg:mb-0 scale-[0.45] md:scale-75 lg:scale-100 sm:scale-60" />
        </button>
      </div>
      <div v-else>
				<div id="video-container" class="aspect-video child:w-full child:h-full rounded-6"/>
			</div>
    </div>
  </template>
